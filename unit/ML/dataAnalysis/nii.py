...
class nii():
    ...
    def pick(self, index, shape, sampleAxis = 13,IMAGE=False):
        ...
        if IMAGE:
            ...
            record = (record-min)/(max-min)*2-1
            
            maxIndex = np.where(record>1)
            minIndex = np.where(record<-1)
    ...
    def AutoPick(self,index, shape,ACT = 6):
        ...
        '''
        record[0,:,:] = rotate(record_sample,shape,0)
        marks[ 0,:,:] = rotate(marks_sample,shape,0)
        
        
        deg = np.random.random()*45
        record[1,:,:] = rotate(record_sample,shape,deg)
        marks[ 1,:,:] = rotate(marks_sample,shape,deg)
        '''
        '''
        record[1,:,:] = np.rot90(record_sample,1)
        marks[ 1,:,:] = np.rot90(marks_sample, 1)
        
        record[2,:,:] = np.rot90(record_sample,2)
        marks[ 2,:,:] = np.rot90(marks_sample, 2)
        
        record[3,:,:] = np.rot90(record_sample,3)
        marks[ 3,:,:] = np.rot90(marks_sample, 3)
        
        record[4,:,:] = record_sample[:,::-1]
        marks[ 4,:,:] = marks_sample[ :,::-1]
        
        record[5,:,:] = record_sample[::-1,:]
        marks[ 5,:,:] = marks_sample[ ::-1,:]
        '''
        return [record,marks]
